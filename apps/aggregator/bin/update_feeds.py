        ...
        for entry in parsed_feed.entries:
            ...
            try:
                if entry.has_key('modified_parsed'):
                    date_modified = datetime.datetime.now()#datetime.datetime.fromtimestamp(time.mktime(entry.modified_parsed))
                elif parsed_feed.feed.has_key('modified_parsed'):
                    date_modified = datetime.datetime.now()#datetime.datetime.fromtimestamp(time.mktime(parsed_feed.feed.modified_parsed))
                elif parsed_feed.has_key('modified'):
                    date_modified = datetime.datetime.now()#datetime.datetime.fromtimestamp(time.mktime(parsed_feed.modified))
                else:
